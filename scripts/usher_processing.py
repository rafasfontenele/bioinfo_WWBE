...
def create_lineage_tree_for_itol():
    ...
    for node in tree.traverse("postorder"):
        if node.up:
            parent = node.up
            if len(parent.get_children()) == 1 and (node.name not in node_list) and (not node.is_leaf()):
...
if __name__ == '__main__':
    ...
    new_path = os.path.join(mainDir, "usher_process", date)
    ...
    edit_all_paths()
    df = pd.read_csv("lineagePaths.txt", sep="\t")
    clade = "clade"
